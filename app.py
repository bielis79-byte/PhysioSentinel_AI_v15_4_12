...
CLOUD_MODEL_KEY_V1540 = 'physiosentinel_motor_hibrido_v15412_xgboost_horizonte_activo.joblib'
CLOUD_TABLE_V1540 = 'physiosentinel_persistent_objects'
CLOUD_BACKUP_TABLE_V1540 = 'physiosentinel_persistent_backups'
...
def _cloud_put_bytes_v1540(object_key, payload, reason='', base_sha=None, force=False):
    """Escribe de forma segura y deja además una copia append-only para recuperación."""
    state = _cloud_state_v1540()
    ...
    try:
        with _cloud_connect_v1540() as con:
            _cloud_init_schema_v1540(con)
            current = con.execute(
                f'SELECT sha256 FROM {CLOUD_TABLE_V1540} WHERE object_key=%s', (str(object_key),)
            ).fetchone()
            ...
            con.execute(f'''INSERT INTO {CLOUD_BACKUP_TABLE_V1540}
                ...
                (backup_id, str(object_key), '15.4.4', sha, len(payload), payload, str(reason), _instance_id_v1540()))
            con.execute(f'''INSERT INTO {CLOUD_TABLE_V1540}
                ...
                (str(object_key), '15.4.4', sha, len(payload), payload, str(reason), _instance_id_v1540()))
        state['connected'] = True; state['last_error'] = ''; state['base_sha'] = sha
        state['last_sync'] = datetime.now(timezone.utc).isoformat(); state['last_reason'] = reason
...
def _cloud_checkpoint_v1540(reason='data_change', min_interval_s=0, force=False):
    ...
    if not force and state.get('base_sha') and local_sha == state.get('base_sha'):
        state['dirty'] = False; state['dirty_reasons'] = []
        return True
    if min_interval_s and state.get('last_sync') and not force:
        try:
...
st.caption("v15.4.12 · Explicit Horizon Probability UI · Nivel 2 XGBoost · Persistent Cloud Database")
...
with st.sidebar:
    ...
    with st.expander("Historial longitudinal v15.4.5 · Persistent Cloud Database – Historical Record Manager", expanded=True):
        ...
        if _cloud_status_1540['configured']:
            ...
            if _cs1.button("Sincronizar histórico con nube", key="cloud_sync_now_v1540"):
                if _cloud_checkpoint_v1540('manual_sync', force=True):
                    st.success("Histórico sincronizado y copia versionada creada en PostgreSQL. Lazy Sync vuelve a caché local.")
                    st.rerun()
                else:
        ...
        st.caption("Lazy Sync: PostgreSQL se consulta al arrancar/recuperar y al guardar cambios persistentes reales; durante la navegación se trabaja con SQLite/RAM local. Los botones fuerzan sincronización o recuperación.")
...
# ============================================================
# v15.4.12 · MOTOR HÍBRIDO XGBOOST + HORIZONTE TEMPORAL EXPLÍCITO + PROBABILIDAD CALIBRADA
...
V153_MODEL_PATH = V153_STORAGE_DIR / 'motor_hibrido_v15412_xgboost_horizonte_activo.joblib'
...
def _cloud_push_active_model_v1540(reason='model_updated'):
    if not V153_MODEL_PATH.exists() or not _persistent_db_url_v1540():
        return False
    ok, _ = _cloud_put_bytes_v1540(CLOUD_MODEL_KEY_V1540, V153_MODEL_PATH.read_bytes(), reason=reason, force=True)
    return bool(ok)
...
def import_v153_model_bytes(raw_bytes):
    ...
    if (not isinstance(bundle,dict) or bundle.get('version') != '15.4.11-xgb-horizon-prob'
        ...
        raise ValueError('Modelo incompatible: el Nivel 2 de v15.4.12 acepta únicamente bundles XGBoost con horizonte temporal explícito, probabilidad calibrada y Test final intocable.')
    joblib.dump(bundle,V153_MODEL_PATH)
    _cloud_push_active_model_v1540('validated_model_imported')
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10 = st.tabs(["1) Segmentar tipo Kubios", "2) HRV", "3) Comparar", "4) No lineales / MSE / Atractor", "5) Poincaré / Grafos", "6) Dashboard", "7) Informe", "8) Exportar", "9) Índices fisiológicos", "10) Predicción fisiológica"])
...
with tab10:
    ...
    st.caption('Motor Nivel 2: XGBRegressor + XGBClassifier calibrado · horizonte temporal explícito (días) · selección inteligente en Train · 70% Train / 15% Validation / 15% Test final intocable · promoción de regresión solo si TEST mejora ≥2%; probabilidad solo si Brier TEST supera baseline. Walk-Forward temporal adicional.')
    if not XGBOOST_AVAILABLE:
    ...
    with st.expander('Persistencia y control del modelo',expanded=False):
        ...
        if _bundle_v153:
            ...
            _pc1.download_button('Descargar modelo XGBoost activo',_model_bio.getvalue(),file_name='motor_hibrido_v15412_xgboost_horizonte_activo.joblib',mime='application/octet-stream')
        if LONGITUDINAL_DB_PATH.exists():
